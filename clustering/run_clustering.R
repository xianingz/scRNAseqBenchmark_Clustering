...
## Read data
## GiniClust need data before selecting highly variable genes.
if (method == "GiniClust2") {
  scefile = gsub("filteredHVG10","full",scefile)
}
print(scefile)
sce <- readRDS(scefile)
...
truth <- data.frame(cell = as.character(rownames(colData(sce))),
                    trueclass = as.character(sce$Group),
...
saveRDS(list(assignments = assignments, timings = timings,
             k_estimates = k_estimates), file = paste0("/home/xianingz/storage/Scbenchmarking/scRNAseqClusteringBenchmarking/",outrds))
